  ...
  *aRetValue = 0;
  ...
  if (InputFilter::IsEnabledForWindow(mWnd)) {
    switch (msg) {
      ...
      case WM_CHAR:
      case WM_SYSCHAR:
        // Block native WM_CHAR too: the message loop calls TranslateMessage
        // BEFORE DispatchMessage, so native WM_KEYDOWN generates a WM_CHAR
        // in the queue before our InputFilter ever sees the WM_KEYDOWN.
        // Without blocking WM_CHAR, every key produces double input (once
        // from the orphaned native WM_CHAR via ProcessCharMessage, once from
        // our WM_MOUSEMUX_KEY handler). Our own WM_CHAR (from TranslateMessage
        // in the WM_MOUSEMUX_KEY handler) is consumed by NativeKey's
        // PeekMessage(PM_REMOVE) before it reaches the message loop.
        return true;
    ...
    case WM_KEYUP: {
      ...
      if (mMouseMuxClient && mMouseMuxClient->IsConnected()) {
        ...
        if (focusedWnd && focusedWnd != this && focusedWnd->mWnd) {
          ::PostMessage(focusedWnd->mWnd, msg, wParam | MOUSEMUX_MARKER, lParam);
          result = true;
          break;
        }
      }
...
WPARAM nsWindow::wParamFromGlobalMouseState() {
  ...
  if (!!InputFilter::MmGetKeyState(VK_CONTROL)) {
    result |= MK_CONTROL;
  }

  if (!!InputFilter::MmGetKeyState(VK_SHIFT)) {
    result |= MK_SHIFT;
  }

  if (!!InputFilter::MmGetKeyState(VK_LBUTTON)) {
    result |= MK_LBUTTON;
  }

  if (!!InputFilter::MmGetKeyState(VK_MBUTTON)) {
    result |= MK_MBUTTON;
  }

  if (!!InputFilter::MmGetKeyState(VK_RBUTTON)) {
    result |= MK_RBUTTON;
  }

  if (!!InputFilter::MmGetKeyState(VK_XBUTTON1)) {
    result |= MK_XBUTTON1;
  }

  if (!!InputFilter::MmGetKeyState(VK_XBUTTON2)) {
    result |= MK_XBUTTON2;
  }
